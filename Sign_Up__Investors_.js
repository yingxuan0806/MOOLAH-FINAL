 ...
 **/
 ...
 async function createInvestor(){
 	const response = fetch("https://bp7mw1emt2.execute-api.ap-southeast-1.amazonaws.com/default/create_InvestorAcc",{
 		method: 'POST',
 		mode: 'cors',
 		headers:{
 	...
 	response.then(res => {
 		...
 		try{
 			if(res.ok === 1){
 				...
 				let base_url = current_url.replace("/Sign_Up__Investors_.html", "");
 				window.location.href = base_url + '/Profile_Page__Investor_.html' + '?name=' + jsonSchema.Name
 			}
 			else{

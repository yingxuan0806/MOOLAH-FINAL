 ...
 **/
 ...
 const inputHandler = function(e, input) {
 	if(input === "Name"){
 		jsonSchema.Name = e.value;
 	}
 	else if(input === "IGN"){
 		jsonSchema.IGN = e.value;
 	}
 	else if(input === "Email"){
 		jsonSchema.Email = e.value;
 	}
 	else if(input === "Password"){
 		jsonSchema.Password = e.value;
 	}
 	else if(input === "ContactNo"){
 		jsonSchema.ContactNo = e.value;
 	}
 	else if(input === "NRIC"){
 		jsonSchema.NRIC = e.value;
 	}
 	else if(input === "Game"){
 		jsonSchema.Game = e.value;
 	}
 	else if(input === "YoutubeHandle"){
 		jsonSchema.YoutubeHandle = e.value;
 	}
 	else if(input === "TwitchHandle"){
 		jsonSchema.TwitchHandle = e.value;
 	}
 	else if(input === "InstaHandle"){
 		jsonSchema.InstaHandle = e.value;
 	}
 }

 function encodeNRICFrontImageFile(element){
 	var file = element.files[0];
 	var reader = new FileReader();
 	reader.onloadend = function() {
 		let url = reader.result;
 		let base64 = url.replace('data:image/jpeg;base64,', '');
 		jsonSchema.ICPhotoFront = base64;
 		console.log(jsonSchema);
 	}
 	reader.readAsDataURL(file);
 }

 function encodeNRICBackImageFile(element){
 	var file = element.files[0];
 	var reader = new FileReader();
 	reader.onloadend = function() {
 		let url = reader.result;
 		let base64 = url.replace('data:image/jpeg;base64,', '');
 		jsonSchema.ICPhotoBack = base64;
 		console.log(jsonSchema);
 	}
 	reader.readAsDataURL(file);
 }
 ...
 async function createGamer(){
 	const response = fetch("https://7h1n4pcxdc.execute-api.ap-southeast-1.amazonaws.com/default/create_GamerAcc",{
 		method: 'POST',
 		mode: 'cors',
 		headers:{
 	...
 	response.then(res => {
 		...
 		try{
 			if(res.ok === 1){
 				...
 				let base_url = current_url.replace("/Sign_Up__Gamer_.html", "");
 				window.location.href = base_url + '/Profile_Page__Gamer_.html' + '?name=' + jsonSchema.Name
 			}
 			else{
 				alert("Invalid NRIC file/String");
 			}
 		} catch (e) {
 			alert("Invalid NRIC file/String");
 		}
